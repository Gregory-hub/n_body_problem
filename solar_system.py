...
Sun = Star(
    ...
    model=LightSource(engine, pos=sun_init_pos * DISTANCE_RATIO, sphere_color=[0.95, 1, 0.25], scale=0.5 * SCALE_RATIO)
)
...
Venus = Planet(
    ...
    model=Sphere(engine, pos=venus_init_pos * DISTANCE_RATIO, color=[0.9, 0.9, 0.9], scale=0.1 * SCALE_RATIO)
)
...
Mars = Planet(
    ...
    model=Sphere(engine, pos=mars_init_pos * DISTANCE_RATIO, color=[0.9, 0.9, 0.9], scale=0.065 * SCALE_RATIO)
)
...
Saturn = Planet(
    ...
    model=Sphere(engine, pos=saturn_init_pos * DISTANCE_RATIO, color=[0.9, 0.9, 0.9], scale=0.45 * SCALE_RATIO)
)
...
UrAnus = Planet(
    ...
    model=Sphere(engine, pos=uranus_init_pos * DISTANCE_RATIO, color=[0.9, 0.9, 0.9], scale=0.4 * SCALE_RATIO)
)
...
Neptune = Planet(
    ...
    model=Sphere(engine, pos=nuptune_init_pos * DISTANCE_RATIO, color=[0.9, 0.9, 0.9], scale=0.35 * SCALE_RATIO)
)
